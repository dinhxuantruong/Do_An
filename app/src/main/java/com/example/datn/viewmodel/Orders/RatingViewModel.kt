...
class RatingViewModel(private val repositoryProduct: repositoryProduct) : ViewModel() {
    private val _isLoading: MutableLiveData<Boolean> = MutableLiveData(false)
    val isLoading: LiveData<Boolean> = _isLoading

    private val _resultRating : MutableLiveData<ResponseResult<ResultMessage>> = MutableLiveData()
    val resultRating : LiveData<ResponseResult<ResultMessage>> get() = _resultRating
  fun  getRatingOrder(id : Int,ratingBody : ratingBody){
      viewModelScope.launch(Dispatchers.IO) {
          try {
              _isLoading.postValue(true)
              val response = repositoryProduct.getRatingOrder(id,ratingBody)
